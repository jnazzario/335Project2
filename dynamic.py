def dynamic(stocks_array, amount, size_of_array):
  ...
  if stocks_array[size_of_array - 1][1] > amount:
    ...
    include_stock = stocks_array[size_of_array - 1][0] + dynamic(
        stocks_array, amount - stocks_array[size_of_array - 1][1],
        size_of_array - 1)
...
with open('DP.txt', 'w') as outfile:
  while i < len(lines):
    ...
    outfile.write(
        f"DP Output: {dynamic(stocks_array, amount, size_of_array)}\n\n")
